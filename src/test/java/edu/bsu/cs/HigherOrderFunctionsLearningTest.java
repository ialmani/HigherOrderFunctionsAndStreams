...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testMap() {
        ...
        List<Integer> actual = input.map(number -> number + 1 )
                .collect(Collectors.toList());
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(string -> string.charAt(0) == 'b')
                .collect(Collectors.toList());
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(string -> string.replaceAll("[^a-zA-Z].*", ""))
                .distinct()
                .collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(string -> string.charAt(string.length() - 1 ) % 2 == 0)
                .reduce("", (string, result) -> string + result);
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(revision -> revision.timestamp.compareTo(instant) < 0)
                .count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input.filter(revision -> whitelist.contains(revision.user))
                .count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision -> !revision.user.toLowerCase()
                .contains("bot"))
                .count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision -> revision.timestamp.atZone(ZoneId.of("UTC"))
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.collect(Collectors.groupingBy(revision -> revision.timestamp.atZone(ZoneId.of("UTC")).getMonth(),
                Collectors.counting()));
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.collect(Collectors.groupingBy(revision -> revision.timestamp.atZone(ZoneId.of("UTC")).getMonth(),
                Collectors.counting()))
                .entrySet()
                .stream()
